...
def create_ontology(united_columns, n_news_total, terms, min_cooccur=1):
	'''
	...
	while check != 0:
    	...
    	tfidf = TfidfVectorizer(ngram_range = (1, 7), min_df = (35/n_news_total), vocabulary = terms)
    	...
    	for word in list(nx.nodes(G1)):
        	for other_word in list(nx.nodes(G1)):
            	if word != other_word and other_word not in visited:
                	if s(word, other_word, docs_words, G1) >= 0.85:
                    	rel_equiv.add(word)
    	...
    	for clust in curr_clusters:
        	...
        	for other in other_clust['cluster']:
            	if other not in visited_clusts:
                	...
                	if single_linkage_distance(curr_clust['word'], another['word'], docs_words, G1) <= (1 / 0.85):
...
def visualise_ontology(united_columns, n_news_total, terms, check_vis_list = False, min_cooccur=1):
	'''
